...
func (r *UsageRepository) UpsertAgentRun(agentID, traceID string, tokens int32) (int64, error) {
	...
	err = r.db.QueryRow(`SELECT id FROM agent_run WHERE trace_id = ?`, traceID).Scan(&existingID)
	switch {
	case err == nil:
		if _, err := r.db.Exec(`UPDATE agent_run SET tokens_used = tokens_used + ? WHERE id = ?`, tokens, existingID); err != nil {
			return 0, err
		}
		return existingID, nil
	case errors.Is(err, sql.ErrNoRows):
		res, err := r.db.Exec(
			`INSERT INTO agent_run (agent_id, channel, trace_id, tokens_used, status, started_at) VALUES (?,?,?,?,?, NOW())`,
...
func (r *UsageRepository) InsertUsageRecord(agentRunID int64, tokens int32) error {
	_, err := r.db.Exec(
		`INSERT INTO usage_record (agent_run_id, tokens, cost_cents, revenue_share) VALUES (?,?,0,0)`,
		agentRunID, tokens,
	)
	return err
}
...
func (r *UsageRepository) SumTokensByAgent(agentID string) (int64, error) {
	...
	err := r.db.QueryRow(
		`SELECT SUM(tokens_used) FROM agent_run WHERE agent_id = ?`, agentID,
	).Scan(&total)
	if err != nil {
		return 0, err
	}
