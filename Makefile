...
help:
	...
	@echo ""
	@echo "各服务目录下都有一个 config.yaml，本地开发默认值已经写好，直接 make dev 即可；"
	@echo "这里的 POSTGRES_DSN / REDIS_ADDR 等变量只是给 make run-xxx 用的可选覆盖，"
	@echo "跟 config.yaml 里的默认值一致（改配置优先改 config.yaml，或用同名环境变量覆盖）。"
